...
export default function InformesPage() {
  const { ventas, gastos, ventaItems, productos, clientes } = useData();
      ...
      {vista === "libro" ? (
        <>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <button onClick={exportarCSV} disabled={conSaldo.length === 0} className="btn-outline ml-auto gap-1.5 text-sm">
        ...
        <div className="mt-4 space-y-6">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="card p-4">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-500">Ticket promedio</p>
              <p className="mt-1 text-2xl font-semibold text-slate-900 dark:text-slate-100">
                {ticket != null ? `S/ ${ticket.toFixed(2)}` : "—"}
              </p>
            </div>
            <div className="card p-4">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-500">Cliente más frecuente</p>
              <p className="mt-1 text-2xl font-semibold text-slate-900 dark:text-slate-100">
          ...
          <div className="card p-4">
            ...
            {topProductos.length === 0 ? (
              <p className="mt-3 text-sm text-slate-400 dark:text-slate-500">Sin ventas con productos asociados todavía.</p>
            ) : (
            ...
            <div className="mt-4 flex items-end gap-3 overflow-x-auto pb-1">
              {meses.map((m) => (
                <div key={m.mes} className="flex flex-1 min-w-[64px] flex-col items-center gap-1">
                  ...
                  <span className="text-[11px] capitalize text-slate-400 dark:text-slate-500">{m.label}</span>
                </div>
              ))}
            </div>
