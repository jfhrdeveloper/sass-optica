...
import { CheckCheck, Plus, Settings2, Trash2 } from "lucide-react";
...
import { puedeCrearRolPersonalizado } from "@/lib/limites-plan";
import { LIMITE_ROLES_PERSONALIZADOS_GRATIS } from "@/lib/precios";
...
export default function RolesPersonalizadosPage() {
  const { empleados, rolesPersonalizados, suscripcion, addRolPersonalizado, updateRolPersonalizado, deleteRolPersonalizado, ready } = useData();
  const toast = useToast();
  const alLimiteRoles = suscripcion?.plan === "gratis" && !puedeCrearRolPersonalizado(suscripcion.plan, rolesPersonalizados.length);
  ...
  return (
    <main>
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <button onClick={abrirNueva} disabled={alLimiteRoles} className="btn-primary h-11 gap-1.5 text-sm disabled:opacity-50 sm:h-auto">
      ...
      {alLimiteRoles && (
        <div className="mt-3">
          <LimitePlanBanner mensaje={`Llegaste a los ${LIMITE_ROLES_PERSONALIZADOS_GRATIS} roles personalizados del plan Gratis.`} />
        </div>
      )}
