      ...
      <ul className="mt-4 space-y-3">
        {notas.map((n) => (
          <li key={n.id} className="border-t border-slate-100 pt-3 text-sm dark:border-slate-800">
            ...
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-500">{n.autor} · {formatearFechaHora(n.createdAt)}</p>
          </li>
        ))}
        {notas.length === 0 && (
          <li className="text-sm text-slate-500 dark:text-slate-500">Sin notas todavía.</li>
        )}
      </ul>
    </div>
  );
