...
- **Modo oscuro:** implementado de verdad (no solo `prefers-color-scheme`) — `@custom-variant dark` (Tailwind v4) + toggle manual (`ThemeToggle.tsx`) vía clase `.dark` en `<html>`, persistido en `localStorage` (clave `tema`), aplicado antes del primer paint por un script inline en `layout.tsx` (evita flash). Todas las clases reutilizables (`.btn-primary`, `.card`, `.input`, `.badge-*`) y las páginas del dashboard/landing/admin-panel tienen su variante `dark:`. Ver bitácora 2026-07-24 (9) para el detalle de la implementación.
  **Default por horario** (sin elección manual guardada): claro de 7:00 a.m. a 6:00 p.m., oscuro el resto — `lib/theme-schedule.ts` calcula la franja con la hora local del dispositivo, y `ThemeScheduler.tsx` reprograma el cambio en vivo mientras la pestaña sigue abierta (sin refrescar). Elegir manualmente con `ThemeToggle.tsx` guarda la preferencia en `localStorage` y esa elección gana siempre sobre el horario, indefinidamente (hasta que se borre el storage).
